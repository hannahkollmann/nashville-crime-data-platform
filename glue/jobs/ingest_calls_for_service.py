"""
ingest_calls_for_service.py

Uploads the full Metro Nashville 911 calls-for-service CSV from the local
`nashville-crime-platform` folder into the S3 data lake:

    s3://nashville-crime-platform/raw/calls_for_service/calls_for_service_2025.csv
"""
...
LOCAL_FILE_NAME = "calls_for_service_2025.csv"
LOCAL_FILE_PATH = LOCAL_BASE_DIR / LOCAL_FILE_NAME

# Target S3 key (path inside the bucket)
S3_KEY = f"raw/calls_for_service/{LOCAL_FILE_NAME}"


def main() -> None:
    print("=== MNPD Calls-for-Service Ingestion (Raw Zone) Started ===")

    # Log a UTC timestamp for the run
    ...
    print("=== Calls-for-Service Ingestion Complete ===")
